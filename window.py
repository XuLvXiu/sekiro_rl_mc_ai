...
class BaseWindow:
    ...
    @staticmethod
    def set_frame(frame):
        print('set new frame:', frame.shape)
...
def set_windows_offset(frame):
    '''
    As we all know, every window in Win 11 has a top bar.
    And the captured screen image contains the whole screen(aka: 1920x1280)
    However, the top bar is not included in the 1280x720 client district of the window.
    '''
